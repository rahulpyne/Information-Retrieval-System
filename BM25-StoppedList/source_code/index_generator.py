...
def write_doc_score(sorted_doc_score):
    try:
        if(len(sorted_doc_score)>0):
            out_file  = open(OUTPUT_FOLDER_PATH+"\\BM25_stoplist_doc_score.txt",'a')
            ...
            out_file.close()
            print "\nDocument Scoring for Query id = " +str(QUERY_ID) +" has been generated inside BM25_stoplist_doc_score.txt"
        else:
...
def start():
    try:
        ...
        if exists(OUTPUT_FOLDER_PATH+"\\BM25_stoplist_doc_score.txt"):
            os.remove(OUTPUT_FOLDER_PATH+"\\BM25_stoplist_doc_score.txt")
